...
public class HolySword extends SwordItem {
    ...
    public HolySword(ToolMaterial toolMaterial, int attackDamage, float attackSpeed, Settings settings) {
        super(toolMaterial, attackDamage, attackSpeed, settings);
    }

    @Override
    public boolean postHit(ItemStack stack, LivingEntity entity, LivingEntity player) {
        entity.addStatusEffect(new StatusEffectInstance(StatusEffects.SLOWNESS, 100, 1, false, false));
        entity.addStatusEffect(new StatusEffectInstance(StatusEffects.WEAKNESS, 100, 1, false, false));
        entity.setOnFireFor(2);
        entity.addStatusEffect(new StatusEffectInstance(StatusEffects.POISON, 100, 1, false, false));
        if (!MinecraftClient.getInstance().player.getItemCooldownManager().isCoolingDown(this)) {
            ...
            TypedActionResult.success(MinecraftClient.getInstance().player.getStackInHand(player.getActiveHand()));
            return true;
        }
        TypedActionResult.fail(MinecraftClient.getInstance().player.getStackInHand(player.getActiveHand()));
        return true;
    }
